...
void Commands::handleTOPIC(int fd, const std::vector<std::string> &args, PollManager &pollManager)
{
    if (args.empty())
    {
        std::string err = ":" + pollManager.getHostname() + " 461 TOPIC :Not enough parameters\r\n";
    ...
    if (args.size() == 1)
    {
        ...
        if (topic.empty())
        {
            std::string msg = ":" + pollManager.getHostname() + " 331 " + client.getNickname() + " " + channelName + " :No topic is set\r\n";
            send(fd, msg.c_str(), msg.size(), 0);
        }
        else
        {
            std::string msg = ":" + pollManager.getHostname() + " 332 " + client.getNickname() + " " + channelName + " :" + topic + "\r\n";
            send(fd, msg.c_str(), msg.size(), 0);
        }
        return;
    }
