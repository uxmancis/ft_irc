...
void Commands::handleKICK(int fd, const std::vector<std::string> &args, PollManager &pollManager)
{
    if (args.size() < 2)
    {
        std::string err = ":" + pollManager.getHostname() + " 461 KICK :Usage: /KICK <#channel> <nickname> [comment]\r\n";
    ...
    {
        std::string err = ":" + pollManager.getHostname() + " 403 " + channelName + " :No such channel\r\n";
        send(fd, err.c_str(), err.size(), 0);
        return;
    }
    ...
    {
        std::string err = ":" + pollManager.getHostname() + " 441 " + targetNick + " " + channelName + " :User is not in the channel\r\n";
        send(fd, err.c_str(), err.size(), 0);
        return;
    }

    std::string kickMsg = ":" + sender.getNickname() + "!user@" + pollManager.getHostname() + " KICK " + channelName + " " + targetNick + " :" + comment + "\r\n";
