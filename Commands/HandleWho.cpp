...
void HandleWHO(int fd, const std::vector<std::string>& args, PollManager& pollManager)
{
    ...
    if (args.empty())
    {
        std::string err = ":irc.local 461 " + requesterNick + " WHO :Not enough parameters\r\n";
    ...
    if (target[0] == '#')
    {
        ...
        {
            std::string endMsg = ":irc.local 315 " + requesterNick + " " + target + " :End of /WHO list\r\n";
        ...
        {
            Client* user = *it;
            std::string msg = ":irc.local 352 " + requesterNick + " " + target + " " +
                user->getUsername() + " localhost irc.local " +
    ...
    else
    {
        ...
        for (it = allClients.begin(); it != allClients.end(); ++it)
        {
            ...
            {
                std::string msg = ":irc.local 352 " + requesterNick + " * " +
                    user.getUsername() + " localhost irc.local " +
                    user.getNickname() + " H :0 " + user.getUsername() + "\r\n";
                send(fd, msg.c_str(), msg.size(), 0);
                break;
